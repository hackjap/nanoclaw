...
- [ ] @bot 멘션으로 에이전트 호출 시 스레드에서 대화 시작
- [ ] AI가 대화형으로 질문하며 Jira 이슈 정보 수집 (제목, 설명, 이슈타입)
- [ ] 수집된 정보로 Jira 이슈 초안 생성 및 미리보기 제시
- [ ] 승인/수정 버튼(Slack interactive components) 제공
- [ ] 승인 시 Jira API를 통해 고정 프로젝트에 이슈 생성
- [ ] 생성된 Jira 이슈 링크를 스레드에 답글로 알림
...
|----------|-----------|---------|
| 스레드 기반 대화 | 채널 noise 최소화, 요청별 컨텍스트 격리 | — Pending |
| 버튼 기반 승인 | 텍스트보다 명확한 UX, 실수 방지 | — Pending |
| 고정 프로젝트 | v1 복잡도 최소화, 추후 확장 가능 | — Pending |
| 기본 필드만 | MVP 빠른 구현, 추후 필드 확장 | — Pending |
